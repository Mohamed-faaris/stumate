...
export function FormsList() {
	...
	const updateFormMutation = useMutation({
		mutationFn: ({ formId, data }: { formId: string; data: EditForm }) => updateForm(formId, data),
		onSuccess: () => {
	...
	const addQuestion = (formId: string, sectionIndex: number) => {
		const newQuestion: Question = {
			questionText: `Question ${(sections[formId]?.[sectionIndex]?.questions?.length || 0) + 1}`,
			type: "SHORT_TEXT",
			required: false,
		};
		setSections((prev) => ({
		...
		setSections((prev) => ({
			...
			[formId]: prev[formId]?.map((section, i) =>
				i === sectionIndex
					? {
							...section,
							questions: section.questions?.map((q, j) => (j === questionIndex ? question : q)),
						}
					: section,
			),
		}));
		...
		updateFormMutation.mutate({
			...
			data: {
				formMeta: {
					...
					description: formsData?.forms?.find((f) => f.id === formId)?.description || "",
					config: {
						type: "Section",
						collapsible: false,
	...
	if (formsError) {
		return (
			<div className="mx-auto max-w-4xl rounded-lg bg-white p-6 shadow-md">
				<h2 className="mb-4 font-bold text-2xl">All Forms</h2>
				<p className="text-red-600">Error loading forms: {formsError.message}</p>
			</div>
		);
	}
	...
	return (
		<div className="mx-auto max-w-4xl rounded-lg bg-white p-6 shadow-md">
			...
			{!formsData?.forms || formsData.forms.length === 0 ? (
				<p className="text-gray-500">No forms found. Create your first form above!</p>
			) : (
						...
						<div key={form.id} className="rounded-lg border p-4">
							<div className="flex items-start justify-between">
								...
								<div className="ml-4 flex space-x-2">
									...
									>
										{editingFormId === form.id ? "Hide Questions" : "Add Questions"}
									</button>
									<button
										type="button"
							...
							{editingFormId === form.id && (
								<div className="mt-6 border-t pt-6">
									<div className="mb-4 flex items-center justify-between">
										<h4 className="font-semibold text-lg">Sections & Questions</h4>
										<button
											type="button"
															...
															<div className="grid grid-cols-1 gap-3 md:grid-cols-2">
																<div>
																	...
																	<input
																		...
																		onChange={(e) =>
																			updateQuestion(form.id, sectionIndex, questionIndex, {
																				...question,
			...
			>
				<DialogContent className="max-h-[80vh] max-w-4xl overflow-y-auto">
					<DialogHeader>
						...
						<DialogDescription>Complete form data fetched from the API</DialogDescription>
					</DialogHeader>
					<div className="mt-4">
